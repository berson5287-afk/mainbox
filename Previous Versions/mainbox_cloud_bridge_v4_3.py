...
BRIDGE_VERSION = "mainbox_cloud_bridge_v4_3_1"
...
# app_state keys that contain Outlook EntryIDs, COM-bound message IDs, or other
# machine-local state that must NOT carry over when restoring to a different machine.
# These rows are deleted from the restored DB immediately after restore so MaINbox
# rebuilds them fresh against the local Outlook profile on first launch.
# Keys ending with '::*' are treated as prefix matches.
_MACHINE_LOCAL_APP_STATE_KEYS: list[str] = [
    # Per-email triage/status dict keyed on raw Outlook EntryIDs (~3-4k entries).
    # EntryIDs are profile-specific; on a different machine they don't exist in COM
    # and MaINbox deadlocks trying to resolve them on startup.
    "email_statuses",
    # Commitment and sent-waiting scanner dedup sets, also keyed on EntryIDs / message-ids
    # that are specific to the machine's Outlook profile. Prefix-matched below.
    "scanner_commitment_processed_message_ids",
...
def sanitize_machine_local_app_state(main_db: Path) -> Dict[str, Any]:
    ...
    try:
        with sqlite3.connect(str(main_db)) as conn:
            # Build exact-match and prefix-match lists
            exact: list[str] = []
            prefixes: list[str] = []
            for key in _MACHINE_LOCAL_APP_STATE_KEYS:
                if "::" in key:
                    prefixes.append(key + "::")
                else:
                    exact.append(key)
...
def run_restore_latest_with_ui(config_path: Path, delay_seconds: float = 0.0) -> int:
    ...
    def finish_ui(ok: bool, error_text: str = "") -> None:
        ...
        if ok:
            ...
            try:
                status_file = Path(config.get("status_file") or default_app_data_dir() / "mainbox_cloud_bridge_status.json")
                prev = read_json_file(status_file, {}) if status_file.exists() else {}
                keep = {
